...
class UDP_GPS_Parser:
    def __init__(self, ip, port, data_type):
        ...
        thread = threading.Thread(target=self.receive_udp_data)
        thread.daemon = True
        thread.start()
    ...
    def data_parsing(self, raw_data):
        ...
        if header[1:] == self.type:
            if self.type == 'GPRMC':
                latitude = int(
                    float(split_str[3])/100)+(float(split_str[3]) % 100)/60
                longitude = int(
                    float(split_str[5])/100)+(float(split_str[5]) % 100)/60
                self.parsed_data = [latitude, longitude]

            if self.type == 'GPGGA':
                latitude = int(
                    float(split_str[2])/100)+(float(split_str[2]) % 100)/60
                longitude = int(
                    float(split_str[4])/100)+(float(split_str[4]) % 100)/60
                self.parsed_data = [latitude, longitude]
