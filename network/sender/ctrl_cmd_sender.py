...
class CtrlCmdSender(Sender):
    def __init__(self, ip, port):
        super().__init__(ip, port)  # Host IP and Port
    ...
    def _format_data(self, data):
        ...
        gear = struct.pack('b', 4)
        # 1: Throttle  /  2: Velocity  /  3: Acceleration
        cmd_type = struct.pack('b', 2)
        velocity = struct.pack('f', data[0])  # cmd_type이 2일때 원하는 속도를 넣어준다.
        acceleration = struct.pack('f', 0)  # cmd_type이 3일때 원하는 가속도를 넣어준다.

        accel = struct.pack('f', 0)
        brake = struct.pack('f', data[1])
        steering = struct.pack('f', data[2])
        message = mode + gear + cmd_type + velocity + \
            acceleration + accel + brake + steering
